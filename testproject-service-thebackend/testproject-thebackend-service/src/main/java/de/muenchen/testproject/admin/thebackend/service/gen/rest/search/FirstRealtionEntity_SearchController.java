...
@ExposesResourceFor(FirstRealtionEntity_.class)
@RequestMapping("/firstRealtionEntitys/search")
public class FirstRealtionEntity_SearchController {
		
    @Autowired
    private QueryService service;

    @Autowired
    private FirstRealtionEntity_Repository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
	    ...
	    Class tmpClass = FirstRealtionEntity_.class;
	    while (tmpClass != null) {
		    annotatedFields.addAll(QueryUtil.getIndexedFields(tmpClass));
		    tmpClass = tmpClass.getSuperclass();
	    }

        Stream<FirstRealtionEntity_> firstRealtionEntityStream;
        try {
            firstRealtionEntityStream = service.query(q, FirstRealtionEntity_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            firstRealtionEntityStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = firstRealtionEntityStream.map(assembler::toResource).collect(Collectors.toList());
    ...
    public ResponseEntity<?> findFullTextJunction(PersistentEntityResourceAssembler assembler, @Param("q") String q)  throws TooManyResultsException, NoSearchableFieldException {
        ...
        try {
            firstRealtionEntityStream = service.queryJunction(q, FirstRealtionEntity_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            firstRealtionEntityStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final Set<PersistentEntityResource> collect = firstRealtionEntityStream.map(assembler::toResource).collect(Collectors.toSet());
