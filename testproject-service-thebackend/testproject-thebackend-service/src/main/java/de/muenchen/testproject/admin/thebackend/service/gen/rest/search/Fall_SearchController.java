...
@ExposesResourceFor(Fall_.class)
@RequestMapping("/falls/search")
public class Fall_SearchController {
		
    @Autowired
    private QueryService service;

    @Autowired
    private Fall_Repository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
	    ...
	    Class tmpClass = Fall_.class;
	    while (tmpClass != null) {
		    annotatedFields.addAll(QueryUtil.getIndexedFields(tmpClass));
		    tmpClass = tmpClass.getSuperclass();
	    }

        Stream<Fall_> fallStream;
        try {
            fallStream = service.query(q, Fall_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            fallStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = fallStream.map(assembler::toResource).collect(Collectors.toList());
    ...
    public ResponseEntity<?> findFullTextJunction(PersistentEntityResourceAssembler assembler, @Param("q") String q)  throws TooManyResultsException, NoSearchableFieldException {
        ...
        try {
            fallStream = service.queryJunction(q, Fall_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            fallStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final Set<PersistentEntityResource> collect = fallStream.map(assembler::toResource).collect(Collectors.toSet());
