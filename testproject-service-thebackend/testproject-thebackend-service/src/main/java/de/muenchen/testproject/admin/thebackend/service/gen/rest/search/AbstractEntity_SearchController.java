...
@ExposesResourceFor(AbstractEntity_.class)
@RequestMapping("/abstractEntitys/search")
public class AbstractEntity_SearchController {
		
    @Autowired
    private QueryService service;

    @Autowired
    private AbstractEntity_Repository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
	    ...
	    Class tmpClass = AbstractEntity_.class;
	    while (tmpClass != null) {
		    annotatedFields.addAll(QueryUtil.getIndexedFields(tmpClass));
		    tmpClass = tmpClass.getSuperclass();
	    }

        Stream<AbstractEntity_> abstractEntityStream;
        try {
            abstractEntityStream = service.query(q, AbstractEntity_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            abstractEntityStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = abstractEntityStream.map(assembler::toResource).collect(Collectors.toList());
    ...
    public ResponseEntity<?> findFullTextJunction(PersistentEntityResourceAssembler assembler, @Param("q") String q)  throws TooManyResultsException, NoSearchableFieldException {
        ...
        try {
            abstractEntityStream = service.queryJunction(q, AbstractEntity_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            abstractEntityStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final Set<PersistentEntityResource> collect = abstractEntityStream.map(assembler::toResource).collect(Collectors.toSet());

		
		tmpClass = FirstLevelEntity_.class;
		...
		try {
			...
			firstLevelEntityStream = StreamSupport.stream(firstLevelEntityRepository.findAll().spliterator(), false);
		}
		
		collect.addAll(firstLevelEntityStream.map(assembler::toResource).collect(Collectors.toList()));
