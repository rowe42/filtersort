...
@ExposesResourceFor(SecondLevelEntity_.class)
@RequestMapping("/secondLevelEntitys/search")
public class SecondLevelEntity_SearchController {
		
    @Autowired
    private QueryService service;

    @Autowired
    private SecondLevelEntity_Repository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
	    ...
	    Class tmpClass = SecondLevelEntity_.class;
	    while (tmpClass != null) {
        ...
        try {
            secondLevelEntityStream = service.query(q, SecondLevelEntity_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            secondLevelEntityStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = secondLevelEntityStream.map(assembler::toResource).collect(Collectors.toList());
    ...
    public ResponseEntity<?> findFullTextJunction(PersistentEntityResourceAssembler assembler, @Param("q") String q)  throws TooManyResultsException, NoSearchableFieldException {
        ...
        final Set<PersistentEntityResource> collect = secondLevelEntityStream.map(assembler::toResource).collect(Collectors.toSet());
