...
@ExposesResourceFor(SecondRealtionEntity_.class)
@RequestMapping("/secondRealtionEntitys/search")
public class SecondRealtionEntity_SearchController {
		
    @Autowired
    private QueryService service;

    @Autowired
    private SecondRealtionEntity_Repository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
	    ...
	    Class tmpClass = SecondRealtionEntity_.class;
	    while (tmpClass != null) {
		    annotatedFields.addAll(QueryUtil.getIndexedFields(tmpClass));
		    tmpClass = tmpClass.getSuperclass();
	    }

        Stream<SecondRealtionEntity_> secondRealtionEntityStream;
        try {
            secondRealtionEntityStream = service.query(q, SecondRealtionEntity_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            secondRealtionEntityStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = secondRealtionEntityStream.map(assembler::toResource).collect(Collectors.toList());
    ...
    public ResponseEntity<?> findFullTextJunction(PersistentEntityResourceAssembler assembler, @Param("q") String q)  throws TooManyResultsException, NoSearchableFieldException {
        ...
        try {
            secondRealtionEntityStream = service.queryJunction(q, SecondRealtionEntity_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            secondRealtionEntityStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final Set<PersistentEntityResource> collect = secondRealtionEntityStream.map(assembler::toResource).collect(Collectors.toSet());
