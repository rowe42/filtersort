...
@ExposesResourceFor(SecondLevelCyclicEntity_.class)
@RequestMapping("/secondLevelCyclicEntitys/search")
public class SecondLevelCyclicEntity_SearchController {
		
    @Autowired
    private QueryService service;

    @Autowired
    private SecondLevelCyclicEntity_Repository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
	    ...
	    Class tmpClass = SecondLevelCyclicEntity_.class;
	    while (tmpClass != null) {
		    annotatedFields.addAll(QueryUtil.getIndexedFields(tmpClass));
		    tmpClass = tmpClass.getSuperclass();
	    }

        Stream<SecondLevelCyclicEntity_> secondLevelCyclicEntityStream;
        try {
            secondLevelCyclicEntityStream = service.query(q, SecondLevelCyclicEntity_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            secondLevelCyclicEntityStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = secondLevelCyclicEntityStream.map(assembler::toResource).collect(Collectors.toList());
    ...
    public ResponseEntity<?> findFullTextJunction(PersistentEntityResourceAssembler assembler, @Param("q") String q)  throws TooManyResultsException, NoSearchableFieldException {
        ...
        try {
            secondLevelCyclicEntityStream = service.queryJunction(q, SecondLevelCyclicEntity_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            secondLevelCyclicEntityStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final Set<PersistentEntityResource> collect = secondLevelCyclicEntityStream.map(assembler::toResource).collect(Collectors.toSet());
